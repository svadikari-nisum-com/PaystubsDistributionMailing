...
public class PaystubsDistributionMailing {
	...
	public static void main(String[] args) {
		File directory = new File("/Paystubs/"+Calendar.getInstance().getDisplayName(Calendar.MONTH, Calendar.LONG_STANDALONE, Locale.ENGLISH)+" "+Calendar.getInstance().get(Calendar.YEAR));
		if(directory!=null) {
	...
	public static boolean sendNotificationMail(File file) {
		...
		try {
			...
			messageBodyPart.setDataHandler(new DataHandler(
					new FileDataSource(file.getPath())));
			messageBodyPart.setFileName(file.getName());
			mp.addBodyPart(messageBodyPart);
			message.setContent(mp);
			message.setSubject("NISUM CONSULTING PVT LTD - PAYSLIP FOR THE MONTH OF "+Calendar.getInstance().getDisplayName(Calendar.MONTH, Calendar.LONG_STANDALONE, Locale.ENGLISH)+" "+Calendar.getInstance().get(Calendar.YEAR));
			InternetAddress[] addressList = new InternetAddress[1];
			addressList[0] = new InternetAddress(file.getName().substring(0, file.getName().lastIndexOf("."))+"@nisum.com");
	...
	private static BodyPart buildHtmlTextPart() throws MessagingException {
		...
		sbBody.append("Hello,<br/><br/>Please find attached here with salary slip for "+Calendar.getInstance().getDisplayName(Calendar.MONTH, Calendar.LONG_STANDALONE, Locale.ENGLISH)+" "+Calendar.getInstance().get(Calendar.YEAR));
